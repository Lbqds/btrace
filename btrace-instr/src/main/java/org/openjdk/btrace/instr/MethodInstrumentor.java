...
public class MethodInstrumentor extends BTraceMethodVisitor {
    ...
    public void visitMethodInsn(int opcode,
        ...
        if (isConstructor() && !prologueVisited) {
            if (name.equals(Constants.CONSTRUCTOR) && (owner.equals(getParentClz()) || (getSuperClz() != null && owner.equals(getSuperClz())))) {
    ...
    protected ValidationResult validateArguments(OnMethod om, Type[] actionArgTypes, Type[] methodArgTypes) {
        ...
        if (om.getSelfParameter() != -1) {
            ...
            if (selfType == null) {
                if (!TypeUtils.isObject(actionArgTypes[om.getSelfParameter()])) {
                    report("Invalid @Self parameter. @Self parameter is not java.lang.Object. Expected " + Constants.OBJECT_TYPE + ", Received " + actionArgTypes[om.getSelfParameter()]);
        ...
        if (om.getReturnParameter() != -1) {
            ...
            if (type == null) {
                if (!TypeUtils.isObject(actionArgTypes[om.getReturnParameter()])) {
                    report("Invalid @Return parameter. @Return parameter is not java.lang.Object. Expected " + Constants.OBJECT_TYPE + ", Received " + actionArgTypes[om.getReturnParameter()]);
                    return ValidationResult.INVALID;
                }
            } else {
        ...
        if (om.getTargetMethodOrFieldParameter() != -1) {
            if (!(InstrumentUtils.isAssignable(actionArgTypes[om.getTargetMethodOrFieldParameter()], Constants.STRING_TYPE, cl, om.isExactTypeMatch()))) {
                report("Invalid @TargetMethodOrField parameter. Expected " + Constants.STRING_TYPE + ", received " + actionArgTypes[om.getTargetMethodOrFieldParameter()]);
                return ValidationResult.INVALID;
            }
            specialArgsCount++;
        }
        if (om.getTargetInstanceParameter() != -1) {
            ...
            if (calledType == null) {
                if (!TypeUtils.isObject(actionArgTypes[om.getTargetInstanceParameter()])) {
                    report("Invalid @TargetInstance parameter. @TargetInstance parameter is not java.lang.Object. Expected " + Constants.OBJECT_TYPE + ", Received " + actionArgTypes[om.getTargetInstanceParameter()]);
                    return ValidationResult.INVALID;
                }
            } else {
                if (!InstrumentUtils.isAssignable(actionArgTypes[om.getTargetInstanceParameter()], calledType, cl, om.isExactTypeMatch())) {
                    report("Invalid @TargetInstance parameter. Expected " + Constants.OBJECT_TYPE + ", received " + actionArgTypes[om.getTargetInstanceParameter()]);
                    return ValidationResult.INVALID;
                }
            }
            specialArgsCount++;
        ...
        if (om.getClassNameParameter() != -1) {
            if (!(InstrumentUtils.isAssignable(actionArgTypes[om.getClassNameParameter()], Constants.STRING_TYPE, cl, om.isExactTypeMatch()))) {
                return ValidationResult.INVALID;
            }
            specialArgsCount++;
        }
        if (om.getMethodParameter() != -1) {
            if (!(InstrumentUtils.isAssignable(actionArgTypes[om.getMethodParameter()], Constants.STRING_TYPE, cl, om.isExactTypeMatch()))) {
                return ValidationResult.INVALID;
            }
            specialArgsCount++;
        }
        ...
        public void doProvide() {
            ...
            for (int j = 0; j < myArgTypes.length; j++) {
                ...
                asm.dup()
                        ...
                        .box(argType)
                        .arrayStore(Constants.OBJECT_TYPE);
