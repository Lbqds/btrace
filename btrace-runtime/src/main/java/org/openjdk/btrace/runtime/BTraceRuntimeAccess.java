...
public abstract class BTraceRuntimeAccess implements RuntimeContext {
     ...
     */
    public static boolean enter(BTraceRuntime.Impl current) {
    ...
    public static String getClientName(String forClassName) {
        ...
        if (idx > -1) {
            name = Auxilliary.class.getPackage().getName().replace('.', '/') + "/" + name.substring(idx + 1);
        } else {
            name = Auxilliary.class.getPackage().getName().replace('.', '/') + "/" + name;
        }
        clients.add(name);
        return name;
    }
     ...
     * if there is one.
     */
    static BTraceRuntime.Impl getCurrent() {
    ...
    public void send(String msg) {
        BTraceRuntime.Impl rt = getCurrent();
        if (rt != null) {
            rt.send(msg);
        }
    }
